...
class Utilities extends Core
{
    ...
    public function fatalError($code, $header, $error_msg){
        die($this->displayError($code, $header, $error_msg));
    }

    public function displayError($code, $header, $error_msg){
        $html = '';
        ...
        if($code){
            $html .= '<h4>Код ошибки <h4><code>' . $code . '</code></p>';
        }

        if($header || $error_msg || $code){
            $html = '<div style="padding: 5px 10px; background: #eee; border: 1px dashed red; font-family: Arial, Sans; font-size: 12px; line-height: 1.75em">' . $html . '</div>';
        }
    ...
    {
        print $this->displayError('DEBUG', 'Debugging', print_r($var, true));
    }
}
